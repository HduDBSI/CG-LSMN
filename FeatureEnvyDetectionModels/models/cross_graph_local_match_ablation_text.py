...
class crossGraphLocalMatch_ablation_text(nn.Module):
    def __init__(self, vocabSize, hidden, in_features, out_features, class_num, dropout, alpha):
        super(crossGraphLocalMatch_ablation_text, self).__init__()
    ...
    def forward(self, h1_index, edge_index1, h2_index, edge_index2):
        ...
        # print("textInfo",textInfo.shape) #torch.Size([1, 256])
        ...
        #fusionOut = self.fusionBiLSTM(torch.stack([textInfo], dim=0))
        #print("fusionOut",fusionOut.shape)
        out = self.fc(textInfo.reshape(1,-1))
        #print("out",out.shape)
