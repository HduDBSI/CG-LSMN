...
def getDataListByProjectList_(dataItemList, train_project_list, test_project_list):
    train_list = []
    test_list = []
...
if __name__ == '__main__':
    ...
    for task in range(2,3):
        if task == 1:
            ...
            vocabDictPath = codaDataPath + "vocabDict_RQ4.json"
            saveVocab2TxtPath = codaDataPath + "vocabDictList_RQ4.txt"
            labelPath = codaDataPath + "labels.txt"
            newLabelPath = codaDataPath + "new_labels.txt"
            saveModelPath = codaDataPath + 'saveModels'
            test_result = saveModelPath + '/result_ours_RQ4.txt'
        elif task == 2:
            codaDataPath = datasetPath + "Dataset_Method_Class/"
            vocabDictPath = codaDataPath + "vocabDict_RQ4_m.json"
            saveVocab2TxtPath = codaDataPath + "vocabDictList_RQ4_m.txt"
            labelPath = codaDataPath + "labels.txt"
            newLabelPath = codaDataPath + "new_labels.txt"
            saveModelPath = codaDataPath + 'saveModels'
            test_result = saveModelPath + '/result_ours_RQ4_m.txt'
        print('__task__:',task, codaDataPath)

        #test_result = saveModelPath + '/result-graphOpt.txt'
        ...
        no_argm_dataItemList = []
        for data in dataItemList:
            if task == 1:
                if data.split()[0].split('/')[1].split('_')[-1] == '0': #数据集只采用原始样本,忽略数据增强的样本
                    no_argm_dataItemList.append(data)
            elif task == 2:
                if '_'.join(data.split()[0].split('/')[1].split('_')[-2:]) == 'pos_0' or data.split()[0].split('/')[1].split('_')[-2] == 'neg': #数据集只采用原始样本,忽略数据增强的样本
                    no_argm_dataItemList.append(data)
                   
        #exit()
        vocabList, vocabDict, newDataItemList, allDataDict = getVocabListByAST(codaDataPath, no_argm_dataItemList, saveVocab2TxtPath, vocabDictPath, newLabelPath)
        ...
        for fold_idx in range(1,6):
            ...
            #获取训练集和测试集
            train_list, test_list = getDataListByProjectList_(newDataItemList, train_project_list, test_project_list)
