...
def getBatchData(train_data, allDataDict, batch_size, batch_index, device):
    ...
    for item in batch_items:
        ...
        if data_name in allDataDict:
            ...
            # names = torch.as_tensor(name_m + name_ec + name_tc).unsqueeze(0).permute(0,2,1).to(device)
            # distance = torch.as_tensor(allDataDict[data_name]["distance"]).reshape(1,2,1).to(device)
            # label = torch.Tensor(allDataDict[data_name]["label"]).to(device)

            names = name_m + name_ec + name_tc
            distance = allDataDict[data_name]["distance"]
            label = allDataDict[data_name]["label"]
            ...
            continue
    #print('batch_names', torch.as_tensor(batch_names).shape)
    #print('batch_distance', torch.as_tensor(batch_distance).shape)
    batch_names = torch.as_tensor(batch_names).permute(0,2,1).to(device)
    #print('batch_names', torch.as_tensor(batch_names).shape)
    batch_distance = torch.as_tensor(batch_distance).to(device)
    #print('batch_distance', torch.as_tensor(batch_distance).shape)
    batch_label = torch.Tensor(batch_label).squeeze().to(device)
    return batch_names, batch_distance, batch_label

class LiuCNN(nn.Module):
    def __init__(self, vec_dim, dis_dim, hidden, kernel_size):
        super(LiuCNN, self).__init__()
        # model_left
        self.hidden = hidden
        self.conv1d_l1 = Conv1d(vec_dim, hidden, kernel_size, padding=0)
        self.conv1d_l2 = Conv1d(hidden, hidden, kernel_size, padding=0)
        self.conv1d_l3 = Conv1d(hidden, hidden, kernel_size, padding=0)
        self.dense_l = Linear(15*hidden, hidden)
        # model right
        self.conv1d_r1 = Conv1d(dis_dim, hidden, kernel_size, padding=0)
        self.conv1d_r2 = Conv1d(hidden, hidden, kernel_size, padding=0)
        self.conv1d_r3 = Conv1d(hidden, hidden, kernel_size, padding=0)
        # merge model
        self.dense1 = Linear(2*hidden, hidden)
        self.dense2 = Linear(hidden, 2)

    def forward(self, names, distance):
        # model_left
        #print('names',names.shape)
        names = torch.tanh(self.conv1d_l1(names))
        names = torch.tanh(self.conv1d_l2(names))
        names = torch.tanh(self.conv1d_l3(names))
        names = self.dense_l(names.view(-1,15*self.hidden))
        #print('names',names.shape)
        ...
        #print('distance',distance.shape)
        distance = torch.tanh(self.conv1d_r1(distance))
        distance = torch.tanh(self.conv1d_r2(distance))
        distance = torch.tanh(self.conv1d_r3(distance))
        distance = distance.view(-1,self.hidden)
        #print('distance',distance.shape)

        # merge model
        merge = torch.cat([names, distance], dim=-1)
        merge = torch.tanh(self.dense1(merge))
        out = torch.sigmoid(self.dense2(merge))
        #print('out',out.shape)

        return out

class LiuDense(nn.Module):
    def __init__(self, vec_dim, dis_dim, hidden, kernel_size):
        super(LiuDense, self).__init__()
        # model_left
        self.conv1d_l1 = Conv1d(vec_dim, hidden, kernel_size, padding=0)
        self.conv1d_l2 = Conv1d(hidden, hidden, kernel_size, padding=0)
        self.conv1d_l3 = Conv1d(hidden, hidden, kernel_size, padding=0)
        self.dense_l = Linear(15*hidden, hidden)
        # model right
        self.dense_r1 = Linear(dis_dim, 16)
        self.dense_r2 = Linear(16, 16)
        self.dense_r3 = Linear(16, 16)
        self.dense_r = Linear(16, hidden)
        # merge model
        self.dense1 = Linear(2*hidden, hidden)
        self.dense2 = Linear(hidden, 2)
    ...
    def forward(self, names, distance):
        ...
        names = torch.tanh(self.conv1d_l2(names))
        names = torch.tanh(self.conv1d_l3(names))
        names = self.dense_l(names.reshape(1,-1)).flatten()

        # model right
        distance = distance.reshape(-1,)
        distance = torch.tanh(self.dense_r1(distance))
        distance = torch.tanh(self.dense_r2(distance))
        distance = torch.tanh(self.dense_r3(distance))
        distance = self.dense_r(distance.reshape(1,-1)).flatten()

        # merge model
        merge = torch.cat([names, distance], dim=-1)
        merge = torch.tanh(self.dense1(merge))
        out = torch.sigmoid(self.dense2(merge)).reshape(1,-1)
        return out
    ...
    with torch.no_grad():
        ...
        for i in Test_data_batches:
            ...
            try:
                ...
                distance = torch.as_tensor(data["distance"]).reshape(1,2).to(device)
                testCount += 1
            except:
                notFound += 1
                continue
...
def train(train_list, test_list, fold_idx):
    ...
    for epoch in epochs:
        ...
        for batch_index in tqdm(range(int(len(train_data)/args.batch_size))):
            batch_names, batch_distance, batch_label = getBatchData(train_data, allDataDict, args.batch_size, batch_index, device)
            #print("batch_data",batch_data)

            output = model(batch_names, batch_distance)
            batchloss = F.binary_cross_entropy(output, batch_label)
            ...
            main_index = main_index + len(batch_names)
...
if __name__ == '__main__':
    ...
    test_result = saveModelPath + '/liu_cnn_result.txt'
    ...
    for fold_idx in range(1,6):
        ...
        model = LiuCNN(200, 2, 128, 1).to(device)
        #model = LiuDense(200, 2, 128, 1).to(device)
