...
from models.cross_graph_local_match_ablation_text import crossGraphLocalMatch_ablation_text
from models.cross_graph_local_match_ablation_text_type import crossGraphLocalMatch_ablation_text_type
...
def train(train_list, test_list, fold_idx):
    ...
    for epoch in epochs:
        ...
        if f1 > f1_max:
            f1_max = f1
            torch.save(model.state_dict(), saveModelPath+'/'+'epoch'+str(epoch)+'.pkl')
...
if __name__ == '__main__':
    ...
    for task in range(2):
        ...
        for myModle in [crossGraphLocalMatch_ablation_text, crossGraphLocalMatch_ablation_text_type]:
            print(str(myModle))
            # exit()
            ...
            test_result = saveModelPath + '/result_'+ str(myModle) + '_task_' + str(task) +'.txt'
            ...
            for fold_idx in range(1,6):
                ...
                #获取训练集和测试集
                if codaDataPath == datasetPath + "Dataset_Class_Class/":
                    task = 1
                elif codaDataPath == datasetPath + "Dataset_Method_Class/":
                    task = 2
                train_list, test_list = getDataListByProjectList(newDataItemList, task, train_project_list, test_project_list)
                
                model = myModle(vocabSize, args.hidden, args.in_features, args.out_features, args.num_classes, args.dropout, args.alpha).to(device)
