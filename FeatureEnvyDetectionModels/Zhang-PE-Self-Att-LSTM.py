...
def getNameWordIndex(name_list):
    name_vec_index = []
    if len(name_list) < 5:
        for i in range(len(name_list)):
            name_vec_index.append(vocabDict[name_list[i]])
        for j in range(5-len(name_list)):
            name_vec_index.append(len(vocabDict))
    else:
        for i in range(5):
            name_vec_index.append(vocabDict[name_list[i]])
    return name_vec_index 
...
def getAllDataDict(datasetPath):
    with open(datasetPath, 'r') as f:
        ...
        for row in tqdm(items):
            ...
            name_m = getNameWordIndex(split_except_alphabetDigitChinese(data[1]))
            name_ec = getNameWordIndex(split_except_alphabetDigitChinese(data[2]))
            name_et = getNameWordIndex(split_except_alphabetDigitChinese(data[3]))
            distance = [float(data[4]),float(data[5])]
            label = [[0,1]] if int(data[6]) == 1 else [[1,0]]
            dataItemDict["names"] = name_m + name_ec + name_et
            # dataItemDict["names"] = [name_m, name_ec, name_et]
            #print(dataItemDict["names"])
...
def getBatchData(train_data, allDataDict, batch_size, batch_index, device):
    ...
    for item in batch_items:
        ...
        if data_name in allDataDict:
            #print("data_name",data_name)
            names = torch.as_tensor(allDataDict[data_name]["names"]).to(device)
...
def getVocabDict(corpusPath, moreCorpusPath):
    corpus = []
    with open(corpusPath, 'r') as f1:
        lines = f1.readlines()
        for line in lines:
            corpus += line.split()
    with open(moreCorpusPath, 'r') as f2:
        lines = f2.readlines()
        for line in lines:
            corpus += line.split()
    corpusSet = list(set(corpus))
    corpusDict = {}
    for index, word in enumerate(corpusSet):
        corpusDict[word] = index
...
class LSTM_Block(nn.Module):
    def __init__(self, in_dim, out_dim, dropout=0.1):
        ...
        self.Dense = Linear(in_dim, out_dim)

    def forward(self, input):
        input = torch.tanh(self.LSTM(input))
        output = torch.tanh(self.Dense(input))
...
class PESelfAttLSTM(nn.Module):
    def __init__(self, vec_dim, dis_dim, vocabSize):
        super(PESelfAttLSTM, self).__init__()
        # model_left
        #input_dim, hidden_dim, layer_dim, output_dim):
        self.embed = nn.Embedding(vocabSize+1,vec_dim)
        self.pos_encoder = PositionalEncoding(vec_dim)
        self.self_Attention = SelfAttentionLayer(200, 200)
        self.lstm_block1 = LSTM_Block(200, 200)
        self.lstm_block2 = LSTM_Block(200, 200)
        self.lstm_block3 = LSTM_Block(200, 200)
        self.conv_block1 = Conv_Block(200, 128)
        self.conv_block2 = Conv_Block(128, 64)
        self.conv_block3 = Conv_Block(64, 32)
        self.dense_l = Linear(15*32, 128)
        
        # model right
        self.conv_block4 = Conv_Block(2, 128)
        self.conv_block5 = Conv_Block(128, 128)
        self.conv_block6 = Conv_Block(128, 128)

        # merge model
        self.dense1 = Linear(256, 1024)
        self.dense2 = Linear(1024, 128)
        self.dense3 = Linear(128, 2)

    def forward(self, names, distance):
        # model_left
        names = self.embed(names)
        names = self.pos_encoder(names.unsqueeze(1)).squeeze(1)
        names = self.self_Attention(names).unsqueeze(1)
        names = self.lstm_block1(names)
        names = self.lstm_block2(names)
        names = self.lstm_block3(names)
        names = self.conv_block1(names.permute(0,2,1))
        names = self.conv_block2(names)
        names = self.conv_block3(names)
        names = self.dense_l(names.reshape(1,-1))
        names = names.flatten()   #torch.Size([128])
        #print("names.flatten()",names.flatten().shape)

        # model right
        #print("distance",distance.shape)
        distance = self.conv_block4(distance)
        distance = self.conv_block5(distance)
        distance = self.conv_block6(distance)
        distance = distance.flatten() #torch.Size([128])
        #print("distance.flatten()",distance.flatten().shape)
        ...
        merge = torch.tanh(self.dense2(merge))
        merge = torch.tanh(self.dense3(merge))
        out = torch.sigmoid(merge.reshape(1,-1))
        #print("modelout",out.shape)
        #exit()
        return out
    ...
    with torch.no_grad():
        ...
        for i in Test_data_batches:
            ...
            try:
                ...
                names = torch.as_tensor(data["names"]).to(device)
...
if __name__ == '__main__':
    ...
    for task in range(1,3):
        ...
        test_result = saveModelPath + '/zhang_PE_att_result.txt'
        ...
        vocabDict = getVocabDict(corpusPath, moreCorpusPath)
        ...
        for fold_idx in range(2,6):
            ...
            model = PESelfAttLSTM(200, 2, vocabSize).to(device)
